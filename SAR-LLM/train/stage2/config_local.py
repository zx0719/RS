...
class TrainConfig:
    ...
    batch_size: int = 4
    lr: float = 2e-5
    lora_lr: float = 2e-5          # LoRA 学习率（与 projector 可分开设置）
    max_steps: int = 10000
    ...
    # 混训权重（0 = 不使用）
    mixed_weight_sarvqa:      float = 1.0   # SARTEXT VQA 多轮对话
    mixed_weight_sartext:     float = 0.5   # SARTEXT 单轮 caption
    mixed_weight_sarlang_vqa: float = 0.0   # SARLANG-1M VQA（运行 get_pt_sarlang_vqa.py 后设为 1.0）
    mixed_seed: int = 42

    # swanlab 日志
    use_swanlab: bool            = True
    swanlab_project: str         = "SARCLIP-Qwen"
    swanlab_experiment_name: str = "stage2_vqa_run"

    # 保存
    save_dir: str  = "/mnt/data/qianwentao/checkpoints_sarqwen_stage2"
